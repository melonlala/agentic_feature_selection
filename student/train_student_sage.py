"""Train BC students with the top-k features from a SAGE ranking.

Ranking source: explain/sage_rank_continuous.py (SAGE permutation estimator
on a full-feature BC student).

Usage:
    python student/train_student_sage.py \\
        --config       configs/kitchen_complete.yaml \\
        --seed         0 \\
        --dataset_path outputs/.../dataset.npz \\
        --ranking_path outputs/rankings_sage/.../ranking.csv \\
        --output_dir   outputs/students/.../sage
"""
...
def main() -> None:
    p = argparse.ArgumentParser(
        description="BC students with SAGE top-k feature selection.",
    )
    ...
    p.add_argument("--ranking_path", required=True,
                   help="Path to ranking.csv produced by explain/sage_rank_continuous.py.")
