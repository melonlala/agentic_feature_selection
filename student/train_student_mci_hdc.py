"""Train BC students with the top-k features from an MCI-HDC ranking.

Ranking source: explain/mci_rank.py (HDC + ridge regression).

Usage:
    python student/train_student_mci_hdc.py \\
        --config       configs/kitchen_complete.yaml \\
        --seed         0 \\
        --dataset_path outputs/.../dataset.npz \\
        --ranking_path outputs/rankings_mci_hdc/.../ranking.csv \\
        --output_dir   outputs/students/.../mci_hdc
"""
...
SELECTOR_NAME = "mci_hdc"


def main() -> None:
    p = argparse.ArgumentParser(
        description="BC students with MCI-HDC top-k feature selection.",
    )
    ...
    p.add_argument("--ranking_path", required=True,
                   help="Path to ranking.csv produced by explain/mci_rank.py.")
