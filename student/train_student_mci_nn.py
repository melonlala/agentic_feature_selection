"""Train BC students with the top-k features from an MCI-NN ranking.

Ranking source: explain/mci_rank_nn.py (per-subset MLP retraining).

Usage:
    python student/train_student_mci_nn.py \\
        --config       configs/kitchen_complete.yaml \\
        --seed         0 \\
        --dataset_path outputs/.../dataset.npz \\
        --ranking_path outputs/rankings_mci_nn/.../ranking.csv \\
        --output_dir   outputs/students/.../mci_nn
"""
...
SELECTOR_NAME = "mci_nn"


def main() -> None:
    p = argparse.ArgumentParser(
        description="BC students with MCI-NN top-k feature selection.",
    )
    ...
    p.add_argument("--ranking_path", required=True,
                   help="Path to ranking.csv produced by explain/mci_rank_nn.py.")
